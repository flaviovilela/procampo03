...
begin
  ...
  BEGIN
    try
      Grupo_Produto.Excluir;
      Mensagem.Exclusao_Realizada;
      Util.Insere_Historico(usuario, 'EXCLUIU GRUPO DE PRODUTO ' + EdtDescricao.Text + '.', TimeToStr(time), exclusao, date);
    except
  ...
  if (achei = false) then
    begin
      ...
      BEGIN
        Gera_Codigo;
        try
          Grupo_Produto.Inserir;
          Mensagem.Confirma_Insercao;
          Util.Insere_Historico(usuario, 'CADASTROU GRUPO DE PRODUTO ' + EdtDescricao.Text + '.', TimeToStr(time), insercao, date);
        except
          Mensagem.Impossivel_Executar;
        end;
      END
    ...
    begin
      ...
      BEGIN
        try
          Grupo_Produto.Alterar;
          Mensagem.Alteracao_Realizada;
          Util.Insere_Historico(usuario, 'ALTEROU GRUPO DE PRODUTO ' + EdtDescricao.Text + '.', TimeToStr(time), alteracao, date);
        except
          Mensagem.Impossivel_Executar;
        end;
      END
...
begin
  try
    ...
    begin
      ...
      clear;
      add('select max(Codigo) as Codigo from Cadastro_Grupo_Produto');
      open;
    end;
